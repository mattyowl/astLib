...
def clipImageSectionWCS(imageData, imageWCS, RADeg, decDeg, clipSizeDeg, returnWCS = True):
    ...
    Args:
        ...
        clipSizeDeg (float or list): if float, size of square clipped section in decimal degrees;
            if list, size of clipped section in degrees as [widthDeg, heightDeg]
        returnWCS (bool): if True, return an updated WCS for the clipped section

    Returns:
        dict: clipped image section (numpy.ndarray), updated WCS object for the clipped image
...
def clipImageSectionPix(imageData, XCoord, YCoord, clipSizePix):
    ...
    Args:
        ...
        clipSizePix (float or list): if float, size of square clipped section in pixels; if list,
            size of clipped section in pixels as [widthPix, heightPix]

    Returns:
        numpy.ndarray: clipped image section
...
def clipRotatedImageSectionWCS(imageData, imageWCS, RADeg, decDeg, clipSizeDeg, returnWCS = True):
    ...
    Args:
        ...
        decDeg (float): dec. coordinate in decimal degrees
        clipSizeDeg (float or list): if float, size of square clipped section in decimal degrees;
            if list, size of clipped section in degrees as [widthDeg, heightDeg] in the RA, dec.
            axes of the output rotated image respectively
        returnWCS (bool): if True, return an updated WCS for the clipped section

    Returns:
        dict or None: clipped image section (numpy.ndarray) and updated WCS object in the format
            ``{'data', 'wcs'}``, or None if the requested position is not found within the image.

    Note:
        If the image WCS does not have keywords of the form CD1_1 etc., the output WCS will not
        be rotated.
...
def clipUsingRADecCoords(imageData, imageWCS, RAMin, RAMax, decMin, decMax, returnWCS = True):
    ...
    Returns:
        dict or None: clipped image section (numpy.ndarray), updated WCS object, and
            corresponding pixel coordinates in imageData in the format
            ``{'data', 'wcs', 'clippedSection'}``, or None if the requested position is not
...
def scaleImage(imageData, imageWCS, scaleFactor):
    ...
    Args:
        imageData (numpy.ndarray): image data array
        imageWCS (astWCS.WCS): WCS object for the image
        scaleFactor (float or list or tuple): factor to resize image by; if tuple or list, in
            format [x scale factor, y scale factor]

    Returns:
        dict: image data (numpy.ndarray) and updated WCS object in format ``{'data', 'wcs'}``
...
def intensityCutImage(imageData, cutLevels):
    ...
    Args:
        ...
        cutLevels (list): sets the image scaling - available options:
            ...
            - pixel values: ``[low value, high value]``
            - histogram equalisation: ``["histEq", number of bins (e.g. 1024)]``
            - relative: ``["relative", cut per cent level (e.g. 99.5)]``
            - smart: ``["smart", cut per cent level (e.g. 99.5)]``

            ``["smart", 99.5]`` seems to provide good scaling over a range of different images.

    Returns:
        dict: image section (numpy.ndarray) and matplotlib image normalisation
            (matplotlib.colors.Normalize) in the format ``{'image', 'norm'}``. If
            ``cutLevels[0] == "histEq"``, only ``{'image'}`` is returned.
...
def resampleToWCS(im1Data, im1WCS, im2Data, im2WCS, highAccuracy = False, onlyOverlapping = True):
    ...
    Args:
        ...
        im2Data (numpy.ndarray): image data array for second image (to be resampled to match
            first image)
    ...
    Returns:
        dict: numpy image data array and associated WCS in format ``{'data', 'wcs'}``
...
def generateContourOverlay(backgroundImageData, backgroundImageWCS, contourImageData, contourImageWCS, \
    ...
    Args:
        ...
        contourLevels (list): sets the contour levels - available options:
            ...
            - linear spacing: ``['linear', min level value, max level value, number of levels]``
              - can use ``"min"``, ``"max"`` to automatically set min, max levels from image data
            - log spacing: ``['log', min level value, max level value, number of levels]``
              - can use ``"min"``, ``"max"`` to automatically set min, max levels from image data
    ...
    Returns:
        dict: resampled contour image array and contour levels in the format
...
def saveContourOverlayBitmap(outputFileName, backgroundImageData, backgroundImageWCS, cutLevels, \
    ...
    Args:
        ...
        contourColor (str): color of the overlaid contours, specified by the name of a standard
...
def saveFITS(outputFileName, imageData, imageWCS = None):
    ...
    Args:
        ...
        imageData (numpy.ndarray): image data array
        imageWCS (astWCS.WCS or None): image WCS object; if None, the FITS image will be
            written with a rudimentary header containing no meta data.
...
def histEq(inputArray, numBins):
    ...
    Returns:
        numpy.ndarray: equalised image data array
...
def normalise(inputArray, clipMinMax):
    ...
    Returns:
        numpy.ndarray: normalised array with minimum value 0, maximum value 1
